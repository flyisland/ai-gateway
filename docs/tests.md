...
-**Important**: To prevent issues with thread safety due capture logs not being [thread-safe](https://github.com/hynek/structlog/blob/d8f4e6ea099a327469e5cd3b015209cb13062335/src/structlog/testing.py#L72),please place the following decorator before the test defintion.

```python
@pytest.mark.xdist_group("capture_logs")
```

The following will wrap all tests within the same group that `pytest-xdist` runs in a single worker.
