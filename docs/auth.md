...
Tokens should be rotated in the following vaults for both staging and production environments. Anyone with vault access can perform these rotations. For additional help, contact the `#g_ai_framework` channel.

| Staging | Production |
|-------------------|---------|------------|
| [AI Gateway](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/staging/service/ai-gateway/) |  [AI Gateway](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/production/service/ai-gateway/) |
| [AI Gateway Custom](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/staging/service/ai-gateway-custom/) |  [AI Gateway Custom](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/production/service/ai-gateway-custom/) |
| [AI Gateway Panda](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/staging/service/ai-gateway-panda/) | | [AI Gateway Panda](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/production/service/ai-gateway-panda/) |

Please start by updating staging first following the steps:

1. Update the `AIGW_SELF_SIGNED_JWT__VALIDATION_KEY` environment variable in the [vault](https://vault.gitlab.net/ui/vault/secrets/runway/kv/list/env/production/service/ai-gateway/) with the current `AIGW_SELF_SIGNED_JWT__SIGNING_KEY` value.
1. Update the value `AIGW_SELF_SIGNED_JWT__SIGNING_KEY` in the vault with a new key. A new key can be generated by the following command: `openssl genrsa -out jwt_signing.key 2048`
