...
- **prompt_version**: Semantic version constraint (e.g., `"^1.0.0"`). If omitted or `null`, uses locally defined prompt
  from flow YAML.
- **response_schema_id**: ID of the response schema from the response schema registry.
- **response_schema_version**: Semantic version constraint (e.g. `"^1.0.0"`).
  - _NOTE: You must provide both `response_schema_id` and `response_schema_version` in order to use response schemas._
...
- **model_size_preference**: Preferred model size for this component. Accepted values are `"small"` or `"large"`.
  When set, the component selects the model mapped to that size bucket in the feature's `models_for_size_preference`
  configuration (see [Model size configuration](../model_selection.md#model-size-configuration)). If the requested
  size is not configured, or if this field is omitted (default: `null`), the feature's `default_model` is used
  instead. Use this to route lightweight tasks (e.g. exploration, summarisation) to a smaller, faster model and
  complex tasks (e.g. planning, code generation) to a larger, more capable model.
...
Unlike prompts, response schemas can only be sourced from the Response Schema Registry. The Response Schema Registry
...
This example demonstrates how to route different tasks to different model sizes for cost and performance optimisation.
A lightweight `"small"` model handles codebase exploration, while a more capable `"large"` model performs the actual
implementation. Both fall back to the feature's `default_model` if the requested size bucket is not configured.
...
components:
    - name: "explorer"
      ...
      prompt_version: "^1.0.0"
      model_size_preference: "small"   # Use a fast, cost-efficient model for exploration
      inputs: ["context:goal"]
      toolset:
          - "read_file"
          - "list_dir"
    ...
    - name: "implementer"
      ...
      prompt_version: "^1.0.0"
      model_size_preference: "large"   # Use a powerful model for complex code generation
      inputs:
          - from: "context:goal"
            as: "goal"
