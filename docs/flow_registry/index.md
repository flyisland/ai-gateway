...
1. A flow MVC should be developed using AI Catalog UI and enabled for some of GitLab projects
1. Once the flow MVC is deemed stable and has sufficient quality, the MVC shall be promoted to a feature and delivered to all GitLab distributions. To do so, one should copy the MVC's YAML config generated by AI Catalog into the AI Gateway repository
