...
### Flow Development process for GitLab team members

Once AI Catalog is ready, the flow development process for GitLab team members will look as follows:

1. A flow MVC should be developed using AI Catalog UI and enabled for some GitLab projects
1. Once the flow MVC is deemed stable and has sufficient quality, the MVC shall be promoted to a feature and delivered to all GitLab distributions. To do so, one should copy the MVC's YAML config generated by AI Catalog into [the AI Gateway repository](https://gitlab.com/gitlab-org/modelops/applied-ml/code-suggestions/ai-assist/-/tree/main/duo_workflow_service/agent_platform/experimental/flows/configs?ref_type=heads)
