...
Generate a dataset with 20 examples using batch size of 5. Smaller batch size helps prevent issues with rate limits and
ensures complete outputs without exceeding token constraints.
...
1. **Template Resolution**: All Jinja templates referenced in the prompt are resolved using `get_message_source()`
   function.
