...
@pytest.mark.asyncio
class TestCodeGenerationsBillingEvents:
    ...
    async def test_billing_event_tracked_on_successful_generation(
        ...
        mock_billing_client.track_billing_event.assert_called_once_with(
            ...
            quantity=25,  # First value from estimate_length mock
        )
    ...
    async def test_no_billing_event_for_streaming(
        self, use_case_with_billing, mock_user, mock_billing_client
    ):
        """Test that billing events are not tracked for streaming responses."""
        ...
        # Billing events should not be tracked for streaming responses
        # since they're handled in the _handle_stream method which doesn't have billing logic
        mock_billing_client.track_billing_event.assert_not_called()
