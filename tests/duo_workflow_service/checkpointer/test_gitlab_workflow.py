...
def mock_compress_checkpoint_flag():
    ...
    with patch(
        ...
        mock_flag.return_value = False  # Default to disabled
        yield mock_flag


class TestCompressCheckpointFeatureFlag:
    """Test cases for COMPRESS_CHECKPOINT feature flag behavior."""
    ...
    async def test_aget_tuple_with_compression_enabled(
        ...
        """Test aget_tuple fetches and decompresses checkpoint when compression is enabled."""
        mock_compress_checkpoint_flag.return_value = True

        # Simulate compressed checkpoint data
        ...
        # Verify compression parameter was added to URL
        http_client.aget.assert_called_once()
        call_path = http_client.aget.call_args[1]["path"]
        assert "compressed=true" in call_path
        assert (
            f"/api/v4/ai/duo_workflows/workflows/{workflow_id}/checkpoints" in call_path
        )

        mock_compress_checkpoint_flag.assert_called_with(
            FeatureFlag.COMPRESS_CHECKPOINT
        )

    @pytest.mark.asyncio
    async def test_aget_tuple_with_compression_disabled(
        self,
        gitlab_workflow,
        http_client,
        config,
        ...
        """Test aget_tuple fetches uncompressed checkpoint when compression is disabled."""
        mock_compress_checkpoint_flag.return_value = False
        ...
        # Verify compression parameter was NOT added to URL
        http_client.aget.assert_called_once()
        call_path = http_client.aget.call_args[1]["path"]
        assert "compressed=true" not in call_path
    ...
    async def test_aget_tuple_per_page_with_compression_enabled(
        ...
        # Verify both per_page and compressed parameters in URL
        call_path = http_client.aget.call_args[1]["path"]
    ...
    async def test_alist_with_compression_enabled(
        ...
        """Test alist fetches and decompresses checkpoints when compression is enabled."""
        mock_compress_checkpoint_flag.return_value = True
        ...
        # Compress the checkpoints
        compressed_checkpoints = [
            {
                **cp,
                "checkpoint": compress_checkpoint(cp["checkpoint"]),
            }
    ...
    @pytest.mark.asyncio
    async def test_alist_with_compression_disabled(
        self,
        gitlab_workflow,
        http_client,
        mock_compress_checkpoint_flag,
    ):
        """Test alist fetches uncompressed checkpoints when compression is disabled."""
        mock_compress_checkpoint_flag.return_value = False
    ...
    async def test_aput_with_compression_enabled(
        ...
        # Verify the checkpoint was compressed in the POST call
        http_client.apost.assert_called_once()
        post_call_body = json.loads(http_client.apost.call_args[1]["body"])

        assert "compressed" in post_call_body
        assert post_call_body["compressed"] is True

        # The checkpoint in the payload should be compressed
        assert post_call_body["checkpoint"] == compress_checkpoint(checkpoint)
    ...
    @pytest.mark.asyncio
    async def test_aput_with_compression_disabled(
        self,
        gitlab_workflow,
        http_client,
        checkpoint_data,
        ...
        """Test aput stores uncompressed checkpoint when compression is disabled."""
        mock_compress_checkpoint_flag.return_value = False
        ...
        assert "compressed" not in post_call_body

        expected_checkpoint = json.loads(json.dumps(checkpoint, cls=CustomEncoder))
        assert post_call_body["checkpoint"] == expected_checkpoint
