...
class TestBuildHierarchyWidget:
    ...
    def test_build_hierarchy_widget_with_different_gid_type(self):
        """Test building hierarchy widget with different GitLab GID type (should still work)."""
        kwargs = {"hierarchy_widget": {"parent_id": "gid://gitlab/Epic/456"}}
        warnings = []

        result = WorkItemBaseTool._build_hierarchy_widget(kwargs, warnings)

        assert result == {"parentId": "gid://gitlab/Epic/456"}
        assert warnings == []
