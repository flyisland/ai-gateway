...
class TestReadFiles:
    @pytest.mark.asyncio
    async def test_read_multiple_valid_files(self):
        mock_outbox = MagicMock()
        mock_outbox.put = AsyncMock()

        # Mock response with content for multiple files
        mock_response = '{"file1.py": {"content": "print(\'hello\')"}, "file2.py": {"content": "print(\'world\')"}}'
        mock_inbox = MagicMock()
        mock_inbox.get = AsyncMock(
            return_value=contract_pb2.ClientEvent(
    ...
    @pytest.mark.asyncio
    async def test_read_files_empty_list(self):
        mock_outbox = MagicMock()
        mock_outbox.put = AsyncMock()

        # Mock response for empty list
        mock_response = "{}"
        mock_inbox = MagicMock()
        mock_inbox.get = AsyncMock(
            return_value=contract_pb2.ClientEvent(
    ...
    @pytest.mark.asyncio
    async def test_read_files_all_errors(self):
        mock_outbox = MagicMock()
        mock_outbox.put = AsyncMock()

        # Mock response with all errors
        mock_response = '{"nonexistent1.py": {"error": "File not found"}, "nonexistent2.py": {"error": "Permission denied"}}'
        mock_inbox = MagicMock()
        mock_inbox.get = AsyncMock(
            return_value=contract_pb2.ClientEvent(
                actionResponse=contract_pb2.ActionResponse(response=mock_response)
        ...
        tool = ReadFiles(description="Read multiple files")
        tool.metadata = metadata
        file_paths = ["nonexistent1.py", "nonexistent2.py"]
    ...
    def test_read_files_format_display_message_empty_list(self):
        tool = ReadFiles(description="Read multiple files")
        input_data = ReadFilesInput(file_paths=[])

        message = tool.format_display_message(input_data)
        assert message == "Read 0 files"
