...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"path": "src/main.py", "ref_name": "main", "first_parent": True},
            123,
...
async def test_create_commit(gitlab_client_mock, metadata, commit_data):
    ...
    expected_response = json.dumps({"status": "success"})
...
async def test_create_commit_branch_params(
    ...
    if expected_start_branch is not None:
        assert commit_params["start_branch"] == expected_start_branch
    else:
        assert "start_branch" not in commit_params
...
async def test_project_info_api_error(gitlab_client_mock, metadata):
    ...
    assert "Failed to resolve default branch" in str(excinfo.value)
