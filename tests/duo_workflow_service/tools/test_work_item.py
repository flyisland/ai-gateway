...
async def test_create_work_item_with_hierarchy_widget(
    ...
    # Verify hierarchy widget was included in GraphQL input
    second_call_args = gitlab_client_mock.graphql.call_args_list[1][0]
    gql_input = second_call_args[1]["input"]
    assert "hierarchyWidget" in gql_input
    assert gql_input["hierarchyWidget"]["parentId"] == "gid://gitlab/WorkItem/456"


@pytest.mark.asyncio
async def test_create_work_item_with_invalid_hierarchy_widget_format(
    gitlab_client_mock, metadata, work_item_type_data
):
    gitlab_client_mock.graphql = AsyncMock()
    gitlab_client_mock.graphql.side_effect = [work_item_type_data]
    ...
    response = await tool._arun(
        ...
        title="Child Work Item",
        type_name="Issue",
        hierarchy_widget="invalid_format",  # Should be a dict
    )

    response_json = json.loads(response)
    assert "work_item" in response_json
    assert "warnings" in response_json
    assert "hierarchy_widget must be a dictionary" in response_json["warnings"]

    # Verify hierarchy widget was not included in GraphQL input
    second_call_args = gitlab_client_mock.graphql.call_args_list[1][0]
    gql_input = second_call_args[1]["input"]
    assert "hierarchyWidget" not in gql_input


@pytest.mark.asyncio
async def test_create_work_item_with_missing_parent_id_in_hierarchy_widget(
    gitlab_client_mock, metadata, work_item_type_data
):
    gitlab_client_mock.graphql = AsyncMock()
    gitlab_client_mock.graphql.side_effect = [work_item_type_data]
    ...
    response = await tool._arun(
        ...
        title="Child Work Item",
        type_name="Issue",
        hierarchy_widget={},  # Missing parent_id
    )

    response_json = json.loads(response)
    assert "work_item" in response_json
    assert "warnings" in response_json
    assert "hierarchy_widget must contain 'parent_id' key" in response_json["warnings"]
...
@pytest.mark.asyncio
async def test_create_work_item_with_invalid_parent_id_format(
    gitlab_client_mock, metadata, work_item_type_data
):
    gitlab_client_mock.graphql = AsyncMock()
    gitlab_client_mock.graphql.side_effect = [work_item_type_data]
    ...
    response = await tool._arun(
        ...
        title="Child Work Item",
        type_name="Issue",
        hierarchy_widget={"parent_id": "invalid_gid_format"},  # Invalid GID format
    )
