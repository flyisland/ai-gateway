...
class TestSanitizeHtmlContent:
    ...
    def test_malformed_attributes_attack_vector(self):
        ...
        result = sanitize_html_content('<div class="valid" onclick="alert(1)" id="test">content</div>')
    ...
    def test_dangerous_script_attributes(self):
        ...
        # onerror handler
        result = sanitize_html_content('<img src="image.jpg" onerror="alert(1)" alt="test">')
        assert result == '<img src="image.jpg" alt="test">'

        # style attribute (potential for CSS injection)
        result = sanitize_html_content('<div style="display:none" hidden>hidden content</div>')
    ...
    def test_unauthorized_attributes_stripped(self):
        ...
        result = sanitize_html_content('<a href="http://example.com" target="_blank">link</a>')
    ...
    def test_authorized_attributes_preserved(self):
        ...
        result = sanitize_html_content('<div class="valid" id="also-valid">normal content</div>')
        assert result == '<div class="valid" id="also-valid">normal content</div>'

        # Link attributes: href, title
        result = sanitize_html_content('<a href="http://example.com" title="Example">link</a>')
        assert result == '<a href="http://example.com" title="Example">link</a>'

        # Image attributes: src, alt, width, height
        result = sanitize_html_content('<img src="image.jpg" alt="test" width="100" height="100">')
        assert result == '<img src="image.jpg" alt="test" width="100" height="100">'

        # Table attributes
        result = sanitize_html_content('<table border="1" cellpadding="5"><tr><td>cell</td></tr></table>')
        assert result == '<table border="1" cellpadding="5"><tr><td>cell</td></tr></table>'
    ...
    def test_html_comments_stripped(self):
        ...
        result = sanitize_html_content('<!-- malicious comment --><p>content</p><!-- another -->')
        assert result == '<p>content</p>'

        # Comments with potential injection
        result = sanitize_html_content('<!--<script>alert(1)</script>--><div>safe</div>')
    ...
    def test_nested_data_structures(self):
        ...
        # List input
        data = [
            '<div this is injected>item1</div>',
            '<p onclick="bad()">item2</p>'
        ]
        result = sanitize_html_content(data)
        expected = [
            '<div>item1</div>',
