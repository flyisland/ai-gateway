...
async def test_run_error_handling(
    ...
    *,
    ...
    workflow_state["conversation_history"]["planner"] = [
        AIMessage(content=[{"type": "text", "text": "test"}], tool_calls=[tool_call], response_metadata={"stop_reason": 'tool_call'}),
    ]
...
async def test_run_error_max_tokens(
    ...
    tool_call = {"id": "3", "name": "read_file", "args": {"file_path": "some-long-file-name-that-is-truncated-"}}
    ...
    workflow_state["conversation_history"]["planner"] = [
        AIMessage(content=[{"type": "text", "text": "test"}], tool_calls=[tool_call], response_metadata={"stop_reason": 'max_tokens'}),
    ]
    ...
    assert len(result) == 2
    assert result[0]["conversation_history"]["planner"][0].content.startswith(
        'Tool read_file raised ToolException: Max tokens reached for tool read_file. Try a simpler request or using a different tool.'
    )
    ...
    assert tool_log["timestamp"] == "2025-01-01T12:00:00+00:00"
    assert tool_log["message_type"] == MessageTypeEnum.TOOL
    assert tool_log["content"].startswith('Failed: Using read_file: file_path=some-long-file-name-that-is-truncated- - Tool call failed: IncompleteToolCallDueToMaxTokens')
    assert tool_log["tool_info"] == ToolInfo(args={'file_path': 'some-long-file-name-that-is-truncated-'}, name='read_file')
