...
def mock_patch_file_fixture(fs: FakeFilesystem):
    """Create a temporary patch file for testing."""
    import json

    path = Path(__file__).parents[3] / "patch_model_selection.json"
    fs.create_file(path, contents=json.dumps({"provider": "gitlab", "name": "gpt_5"}))


class TestModelMetadataHeaderBasedInterceptor:
    @pytest.mark.asyncio
    ...
    async def test_processing_scenarios(
        self, metadata_value, expected_data, test_case, mock_user
    ):
    ...
    async def test_no_processing_scenarios(self, invocation_metadata, test_case):
...
class TestModelMetadataInterceptor:
    ...
    async def test_model_metadata_interceptor_delegation(
        ...
        with patch.object(
            ...
            mock_intercept.assert_called_once_with(continuation, handler_call_details)
