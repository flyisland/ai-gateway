...
def mock_monitoring_fixture():
    ...
    with patch(
        "duo_workflow_service.agent_platform.v1.components.agent.nodes.agent_node.duo_workflow_metrics"
    ) as mock_metrics:
...
def mock_approximate_token_counter_fixture():
    ...
    with patch(
        ...
        mocked_token_counter.count_messages_tokens.return_value = 80
