...
class TestDeterministicStepNodeOutputMappings:
    """Test suite for the output_mappings feature of DeterministicStepNode."""

    @pytest.mark.asyncio
    async def test_run_success_with_output_mappings(
        self,
        workflow_state,
        inputs,
        mock_tool,
        ...
        """Test that output_mappings correctly extracts and maps values to the state."""
        tool_response = {"user_id": "usr_123", "status": "active"}
        mock_tool.arun = AsyncMock(return_value=tool_response)
        sanitized_response = {"user_id": "usr_***", "status": "active"}
        mock_prompt_security.apply_security_to_tool_response.return_value = (
            sanitized_response
        )

        output_mappings = {
            "context:extracted_user_id": "user_id",
            "context:some_other_component:status": "status",
        }

        node = DeterministicStepNode(
            name="test_node_with_mapping",
            tool_name="test_tool",
            inputs=inputs,
        ...
        result = await node.run(workflow_state)

        assert result[FlowStateKeys.CONTEXT]["extracted_user_id"] == "usr_123"
        assert result[FlowStateKeys.CONTEXT]["some_other_component:status"] == "active"
        assert result[FlowStateKeys.CONTEXT]["responses"] == sanitized_response
        assert result[FlowStateKeys.CONTEXT]["status"] == "success"
        mock_prompt_security.apply_security_to_tool_response.assert_called_once_with(
            response=tool_response, tool_name="test_tool"
        )

    @pytest.mark.asyncio
    async def test_output_mappings_with_non_dict_response(
        self,
        workflow_state,
        inputs,
        mock_tool,
        ...
        """Test that output_mappings is skipped for non-dictionary tool responses."""
        mock_tool.arun = AsyncMock(return_value="just a string response")
        mock_prompt_security.apply_security_to_tool_response.return_value = (
            "sanitized string"
        )
        output_mappings = {"context:some_key": "key_that_doesnt_exist"}
        ...
        assert "some_key" not in result.get(FlowStateKeys.CONTEXT, {})
        assert result[FlowStateKeys.CONTEXT]["responses"] == "sanitized string"
        assert result[FlowStateKeys.CONTEXT]["status"] == "success"

    @pytest.mark.asyncio
    async def test_output_mappings_key_not_found_in_response(
        self,
        workflow_state,
        inputs,
        mock_tool,
        ...
        """Test that mapping is skipped for keys not present in the tool response."""
        tool_response = {"actual_key": "some_value"}
        mock_tool.arun = AsyncMock(return_value=tool_response)
        mock_prompt_security.apply_security_to_tool_response.return_value = (
            tool_response
        )
        output_mappings = {"context:non_existent": "key_that_is_missing"}
        ...
        assert "non_existent" not in result.get(FlowStateKeys.CONTEXT, {})
        assert result[FlowStateKeys.CONTEXT]["responses"] == tool_response
        assert result[FlowStateKeys.CONTEXT]["status"] == "success"
