...
@pytest.mark.asyncio
async def test_cleanup_closes_http_client():
    """Test that cleanup properly closes the HTTP client."""
    middleware = EntitlementsMiddleware(
        app=AsyncMock(),
        customersdot_url="https://customers.gitlab.local",
        skip_endpoints=[],
...
@pytest.mark.asyncio
async def test_entitlement_decision_enum_values():
    """Test EntitlementDecision enum values."""
    assert EntitlementDecision.AUTHORIZED.value == "authorized"
    assert EntitlementDecision.NOT_ENTITLED.value == "not_entitled"
    assert EntitlementDecision.ERROR_FALLBACK.value == "error_fallback"


@pytest.mark.asyncio
async def test_entitlement_check_result_dataclass():
    """Test EntitlementCheckResult dataclass structure."""
    authorized_request = EntitlementCheckResult(decision=EntitlementDecision.AUTHORIZED)
    assert authorized_request.decision == EntitlementDecision.AUTHORIZED
    assert authorized_request.error_message is None
    assert authorized_request.should_cache is True

    error_fallback_request = EntitlementCheckResult(
        decision=EntitlementDecision.ERROR_FALLBACK,
        error_message="Test error",
        should_cache=False,
    )
    assert error_fallback_request.decision == EntitlementDecision.ERROR_FALLBACK
    assert error_fallback_request.error_message == "Test error"
    assert error_fallback_request.should_cache is False
