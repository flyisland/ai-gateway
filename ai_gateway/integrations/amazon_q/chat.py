...
class ChatAmazonQ(BaseChatModel):
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            yield ChatGenerationChunk(
                message=AIMessageChunk(content=f"Error: {error_message}")
            )
            ...
            yield ChatGenerationChunk(
                message=AIMessageChunk(content=f"Error ({error_code}): {error_text}")
            )

        except RequestException as e:
            error_message = f"Error communicating with Amazon Q: {str(e)}"
            yield ChatGenerationChunk(
                message=AIMessageChunk(content=f"Connection Error: {error_message}")
            )

        except Exception as e:
            error_message = f"Unexpected error occurred: {str(e)}"
            yield ChatGenerationChunk(
                message=AIMessageChunk(content=f"Internal Error: {error_message}")
            )
        finally:
    ...
    def _build_response(self, messages: List[BaseMessage]):
        try:
            ...
            send_message_params = self._calculate_send_message_params(
    ...
    def _calculate_send_message_params(
