...
class ChatToolsExecutor:
    ...
    async def run(self, state: ChatWorkflowState) -> dict:
        ...
        # Return the entire conversation history including the new tool messages
        # This works with the ChatWorkflowState's _chat_conversation_history_reducer
        # which replaces the entire conversation history
        updated_conversation_history = conversation_history + tool_responses

        return {
            "conversation_history": {
                self._component_name: updated_conversation_history,
            },
        }
