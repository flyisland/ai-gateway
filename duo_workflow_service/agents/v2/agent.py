...
class AgentPromptTemplate(Runnable[DuoWorkflowStateType, PromptValue]):
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        inputs['is_anthropic_model'] = self.template_kwargs.get("is_anthropic_model", False)
    ...
    def _conversation_preamble(
        ...
        rendered_content = jinja2_formatter(
            prompt_template["system_static"],
            is_anthropic_model=self.template_kwargs.get("is_anthropic_model", False),
            handover_tool_name=state.get("handover"),
        )
        
        if "system_static" in prompt_template:
            if isinstance(rendered_content, str) and rendered_content.strip().startswith(('[', '{')):
                parsed = json.loads(rendered_content)
                if isinstance(parsed, list):
                    message = SystemMessage(content=parsed)
                else:
                    message = SystemMessage(content=rendered_content)
            else:
                message = SystemMessage(content=rendered_content)
        ...
        if "system_dynamic" in prompt_template:
            message = SystemMessage(
                content=jinja2_formatter(
                    ...
                    **{
                        k: state[k]
                        for k in ["project", "additional_context"]
        ...
        if "user" in prompt_template:
            message = HumanMessage(
                jinja2_formatter(prompt_template["user"], goal=state.get("goal"))
            )
            conversation_preamble.append(message)
