...
class IncompleteToolCallDueToMaxTokens(Exception):
    """Raised when a tool call is incomplete,
    e.g., due to streaming ending due to max_tokens."""
...
class ToolsExecutor:
    ...
    async def run(self, state: DuoWorkflowStateType):
        ...
        for tool_call in tool_calls:
            ...
            result = await self._execute_tool(tool_name, tool_call, plan, last_message.response_metadata.get('stop_reason'))
    ...
    async def _execute_tool(
            self, tool_name: str, tool_call: ToolCall, plan: Plan, stop_reason: str
    ) -> Dict[str, Any]:
        ...
        except IncompleteToolCallDueToMaxTokens as error:
            return self._handle_tool_error(tool_name, tool_args, error, chat_logs)
