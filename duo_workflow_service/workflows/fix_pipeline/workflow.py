...
from duo_workflow_service.workflows.fix_pipeline.prompts import (
...
class Workflow(AbstractWorkflow):
    ...
    def _setup_workflow_graph(
        ...
        job_iid = self._fetch_job_iid(goal)
        merge_request_title = f"Draft: Fix Job #{job_iid}"
        merge_request_description = f"Fix for failing [job #{job_iid}]({goal})"
            ...
            RunToolNode[WorkflowState](
                ...
                input_parser=lambda _: [
                    ...
                    },
                    {
                        "repository_url": self._project["http_url_to_repo"],  # type: ignore[index]
                        "command": "commit",
                        "args": f"-m 'Duo Workflow: Fix Job #{job_iid}'",
                    },
                    {
                        "repository_url": self._project["http_url_to_repo"],  # type: ignore[index]
                        "command": "push",
                        "args": (
                            ...
                            f"-o merge_request.title={shlex.quote(merge_request_title)} "
                            f"-o merge_request.description={shlex.quote(merge_request_description)}"
                        ),
                    },
    ...
    def _fetch_job_iid(self, job_url: str):
        try:
            gitlab_host = GitLabUrlParser.extract_host_from_url(
                self._project["web_url"]  # type: ignore[index]
            )
            _, job_iid = GitLabUrlParser.parse_job_url(job_url, gitlab_host)
            return job_iid
