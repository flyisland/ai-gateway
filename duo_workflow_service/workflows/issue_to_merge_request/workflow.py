...
from duo_workflow_service.gitlab.gitlab_issue import fetch_gitlab_issue
...
class Workflow(AbstractWorkflow):
    ...
    async def _compile(
        ...
        graph = await self._setup_workflow_graph(
            graph,
            tools_registry,
            goal,
        )
    ...
    async def _setup_workflow_graph(
        self,
        graph: StateGraph,
        tools_registry,
        goal,
        ...
        gitlab_issue = await fetch_gitlab_issue(
            client=self._http_client, issue_url=goal, project=self._project
        )

        if not gitlab_issue.get("description"):
            raise RuntimeError(
                f"Failed to extract gitlab issue from issue_url for workflow {self._workflow_id}"
            )

        # Add nodes to the graph
        graph.set_entry_point("build_context")

        # issue_description = gitlab_issue["description"]
        ...
        merge_request_title = f"Draft: Resolve {gitlab_issue['title']}"
            ...
            RunToolNode[WorkflowState](
                ...
                input_parser=lambda _: [
                    ...
                    },
                    {
                        "repository_url": self._project["http_url_to_repo"],
                        "command": "push",
                        "args": f"-o merge_request.create -o merge_request.title='{merge_request_title}'",
                    },
                ],
