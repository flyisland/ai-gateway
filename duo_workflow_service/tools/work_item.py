...
class WorkItemBaseTool(DuoBaseTool):
    ...
    def _validate_parent_url(
        ...
        try:
            if url:
                ...
                parser = (
                    GitLabUrlParser.parse_group_url
                    if parent_type == "group"
                    else GitLabUrlParser.parse_project_url
                )
                path = parser(url, self.gitlab_host)
                return ResolvedParent(
                    type=parent_type, full_path=self._decode_path(path)
                )

            if group_id:
                return ResolvedParent(
                    type="group", full_path=self._decode_path(str(group_id))
                )

            if project_id:
                return ResolvedParent(
                    type="project", full_path=self._decode_path(str(project_id))
                )
    ...
    def _validate_work_item_url(
        ...
        try:
            if url and "/-/work_items/" in url:
                parsed = GitLabUrlParser.parse_work_item_url(url, self.gitlab_host)
                return ResolvedWorkItem(
                    parent=ResolvedParent(
                        type=parsed.parent_type,
                        full_path=self._decode_path(parsed.full_path),
                    ),
                    work_item_iid=parsed.work_item_iid,
                )
