...
class ListSecurityFindingsInput(BaseModel):
    ...
    severity: Optional[list[SecurityFindingSeverity]] = Field(
        default=None,
        description="Filter by severity levels (e.g., [CRITICAL, HIGH]). If not specified, all severities returned.",
    )
    report_type: Optional[list[SecurityFindingReportType]] = Field(
        default=None,
    ...
    state: Optional[list[SecurityFindingState]] = Field(
        default=None,
...
class ListSecurityFindings(DuoBaseTool):
    ...
    **Do NOT use this tool to list vulnerabilities for an entire project; use 'list_vulnerabilities' for that.**
    ...
    def _prepare_state_filter(
        self, include_dismissed: bool, state: Optional[list[SecurityFindingState]]
    ) -> Optional[list[SecurityFindingState]]:
    ...
    def _build_query_variables(
        ...
        per_page: int,
        cursor: Optional[str],
        severity: Optional[list[SecurityFindingSeverity]],
        report_type: Optional[list[SecurityFindingReportType]],
        scanner: Optional[list[str]],
        state: Optional[list[SecurityFindingState]],
    ) -> dict[str, Any]:
        ...
        if state:
            variables["state"] = [s.value for s in state]
    ...
    def _compute_summary(
        ...
        ai_resolvable = sum(
            1 for f in findings if f.get("aiResolutionAvailable", False)
        )
    ...
    async def _fetch_findings(
        ...
        while True:
            ...
            response = await self.gitlab_client.apost(
                path="/api/graphql",
                body=json.dumps(
                    {"query": LIST_SECURITY_FINDINGS_QUERY, "variables": variables}
                ),
            )
    ...
    async def _arun(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            return json.dumps(
                {
                    ...
                    "metadata": {
                        ...
                        "filters_applied": {
                            ...
                            "report_type": (
                                [rt.value for rt in report_type]
                                if report_type
                                else None
                            ),
            ...
            return json.dumps(
                {
                    "error": f"Failed to list security findings: {str(e)}",
                    "project": project_path,
    ...
    def format_display_message(
        ...
        if args.report_type:
            filters.append(f"type: {', '.join([rt.value for rt in args.report_type])}")
        if args.state:
            filters.append(f"state: {', '.join([s.value for s in args.state])}")
