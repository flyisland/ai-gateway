...
class GetSecurityFindingDetails(DuoBaseTool):
    ...
    **Do NOT use this tool for numeric vulnerability IDs; use the 'get_vulnerability_details' tool for that.**
    ...
    async def _arun(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            return json.dumps(
                {
                    "error": f"An unexpected error occurred while fetching the security finding: {str(e)}",
                    "uuid": uuid,
    ...
    async def _fetch_finding_from_pipeline(
        self, project_path: str, pipeline_id: str, uuid: str
    ) -> str:
        """Fetch a security finding from a specific pipeline using its GID."""
        variables = {
        ...
        if not pipeline:
            return json.dumps(
                {"error": "Pipeline not found", "pipeline_id": pipeline_id}
            )
