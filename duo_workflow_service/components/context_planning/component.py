...
def _router(
    ...
    last_message = state["conversation_history"]["context_planning"][-1]
...
class ContextPlanningComponent(BaseComponent):
    def __init__(
        self,
        combined_toolset: Any,
        executor_toolset: Any,
        project: Project,
        **kwargs: Any,
    ):
        self.combined_toolset = combined_toolset
    ...
    def attach(
        ...
        combined_toolset = self.combined_toolset
        context_planning_agent = self.prompt_registry.get_on_behalf(
            self.user,
            "workflow/context_planning",
            "^1.0.0",
            tools=combined_toolset.bindable,  # type: ignore[arg-type]
            ...
            prompt_template_inputs={
                ...
                # "handover_tool_name": self.tools_registry.get("handover_tool").name,  # type: ignore
        ...
        graph.add_node("context_planning", context_planning_agent.run)
        # graph.add_node("context_planning", context_planning_agent)

        tools_executor = ToolsExecutor(
            tools_agent_name="context_planning",
            toolset=combined_toolset,
            workflow_id=self.workflow_id,
            workflow_type=self.workflow_type,
        )
        context_planning_supervisor = PlanSupervisorAgent(
            supervised_agent_name="context_planning"
        )
        ...
        if approval_component is not None:
            plan_approval_entry_node = approval_component.attach(
                graph=graph,
                next_node=next_node,
                back_node="context_planning",
                exit_node=exit_node,
            )

        graph.add_node("context_planning_tools", tools_executor.run)
        graph.add_node("context_planning_supervisor", context_planning_supervisor.run)

        graph.add_conditional_edges(
            "context_planning",
            _router,
            {
                Routes.CALL_TOOL: "context_planning_tools",
                Routes.SUPERVISOR: "context_planning_supervisor",
                Routes.HANDOVER: plan_approval_entry_node,
                Routes.STOP: exit_node,
            },
        )
        graph.add_edge("context_planning_tools", "context_planning")
        graph.add_edge("context_planning_supervisor", "context_planning")

        return "context_planning"  # entry node for context planning
